...
# The CORS headers an ERROR response needs so the browser can READ it. The catch-all 500
# below is generated by Starlette's ServerErrorMiddleware, which wraps the app OUTSIDE
# CORSMiddleware, so without this a genuine 500 (e.g. a transient Supabase/Cloudflare outage
# that makes a query raise) carries no Access-Control-Allow-Origin and the browser reports it
# as a CORS block instead of the governed "try again" body. We mirror the configured
# allowlist: echo the request Origin ONLY when it is allowed (never "*" with credentials).
# Normal 2xx/4xx responses are unaffected; CORSMiddleware still adds these there.
