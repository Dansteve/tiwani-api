...
**Migrations `0001`-`0018` are ALL APPLIED to the production Supabase database**, which
is the database of record. `0001`-`0008` were applied earlier (see `Docs/Deploy.md`,
...
multi-recipient go-live (the sole-child backfill verified, 0 nulls). `0012` brings the
live `waitlist` table (originally a Supabase-dashboard live edit) under version control
and is an idempotent no-op against the production table. `0013`-`0018` (account
soft-delete, the strategy library, the `recipient_membership` substrate, Shared-Child
sharing, the Village Hub, and the subscription + entitlement foundation) were applied
**2026-06-12** (owner-authorised, each in its own txn): 11 new tables all RLS-on, 20
functions (every SECURITY DEFINER one with `search_path=""`), the subscription seed
loaded (`plan_tier` 3 / `feature_entitlement` 9), and the 4 pre-existing children
backfilled with their creator's owner `recipient_membership`. Migrations are applied
via a direct Postgres connection (`DATABASE_URL` + asyncpg), run locally by the owner,
not on Render.

The per-row `PENDING OWNER APPLY` notes below and the in-file `.sql` banners are now
HISTORICAL (they record each migration's posture at build time, before the 2026-06-12
apply). `0018` has no live Stripe keys yet, so the api's Stripe calls are stubbed
(PENDING OWNER STRIPE KEYS); see the `0018` row. The sharing/village DATA surface is
live on the DB, but the DPIA extension + the psychiatrist/doctor copy sign-off STILL
gate any NON-owner family using it.
...
surface is `/api/v1`, backed solely by the tables in this directory.
...
| `migrations/0016_shared_child_sharing.sql` | The Shared-Child sharing MVP feature layer over the `0015` substrate (`Docs/FeatureDecisions.md`, the Shared-Child REFINE entry, refinements 1/5/7): `share_consent` (first-class, visible, retained per-recipient consent: `recipient_id` -> `child_profile`, `consented_by` -> `auth.users`, `subject_kind` in child/adult, the verbatim governed `consent_text`); the `SECURITY DEFINER` function `get_recipient_card_for_member(child_id)` (the visibility CEILING: returns the recipient's latest live, non-revoked Continuity Card content as jsonb to an ACTIVE MEMBER only, gated on `tiwani_private.is_child_member` at the viewer threshold, the account-bound analogue of `get_card_by_token`); and the `SECURITY DEFINER` RPCs `record_share_consent(child_id, subject_kind, consent_text)` (owner-checked) and `share_recipient_invite(child_id, email, role, token, subject_kind, consent_text, ttl)` (the consent-gated share: for a CHILD it records the consent atomically THEN mints via `mint_recipient_invite`; for an ADULT it BLOCKS unless a recorded adult consent already exists). | APPLIED TO PRODUCTION 2026-06-12 (owner-authorised, `DATABASE_URL`+asyncpg; applied AFTER 0015, on which it depends via `tiwani_private.is_child_member` + `public.mint_recipient_invite`). The VIEWER sees ONLY the Continuity Card, never the raw profile / LCI / alerts: `get_recipient_card_for_member` reads `card_record` only and is membership-gated in SQL, while `child_profile` / `lci_snapshot` / `alert_record` / `pulse_record` keep their owner-only RLS UNCHANGED (a viewer's id is never OR-ed into them). `share_consent` has a member-reads select policy and NO user write policy (consent is written only by the owner-checked RPCs and is never edited/deleted, the retained posture). The adult-recipient share is blocked at the api for the MVP unless the adult has recorded their own consent (refinement 5). RLS + the ceiling proven by `tests/test_shared_child_rls.py` (real-Postgres: viewer-reads-card / viewer-cannot-read-profile-LCI-alerts-pulse / viewer-cannot-write / revoked-viewer-reads-nothing / non-member-reads-nothing / token-cannot-replay / child-share-records-consent / adult-share-blocked-without-consent). The user-facing copy is GOVERNED (`app/engines/sharing`, guard test `tests/test_engine_sharing_guard.py`). |
| `migrations/0015_recipient_membership.sql` | The shared per-recipient sharing substrate: `recipient_membership` (`recipient_id` -> `child_profile`, `user_id` -> `auth.users`, `role` in owner/viewer/editor, `granted_by`, `granted_at`, nullable `revoked_at`, an active-only partial-unique on `(recipient_id, user_id)`); `recipient_invite` (email-bound, single-use `redeemed_at`, short-lived `expires_at`, owner-revocable, unique opaque `token`, role in viewer/editor); the non-exposed `tiwani_private` schema holding `is_child_member(child_id, min_role)` (SECURITY DEFINER, `auth.uid()`-derived, active-only, role-threshold); SPLIT read/write RLS (member-reads select at the viewer threshold, owner-only update; NO user INSERT/DELETE policy); and three SECURITY DEFINER write RPCs `bootstrap_recipient_owner` / `mint_recipient_invite` / `redeem_recipient_invite` (atomic, owner-gated mint, first-wins email-bound redeem). | APPLIED TO PRODUCTION 2026-06-12 (owner-authorised, `DATABASE_URL`+asyncpg; the substrate, applied before 0016/0017). `create_child` mints the creator's owner row via `bootstrap_recipient_owner`, and the 4 pre-existing children were backfilled. The single primitive BOTH Shared-Child sharing and the Village Hub consume (`Docs/FeatureDecisions.md`: the Shared-Child REFINE entry refinements 2/3/4/8 + the Village Hub entry refinement 4); built ONCE, the Hub is the second consumer. is_child_member lives in the NON-exposed `tiwani_private` schema and runs SECURITY DEFINER so the membership policies can ask "is the caller a member" without a policy on `recipient_membership` reading `recipient_membership` (the recursive-policy trap). Reads go to any active member, writes are owner-only at the DB and reachable ONLY through the RPCs (no user write policy on either table, so a viewer can never self-grant or be OR-ed into the owner surface). Invites match the vulnerable-data bar (email-bound, single-use, expiring, owner-revocable, minted/redeemed only by the atomic SECURITY DEFINER RPCs, the 0007 token precedent); a used/revoked/expired/wrong-email token cannot be replayed. Attribution stays SEPARATE from authority: `child_profile.user_id` is the immutable creator, the owner ROLE is a `recipient_membership` row. RLS proven by `tests/test_recipient_membership_rls.py` (viewer-reads / viewer-cannot-write / revoked-viewer-reads-nothing / non-member-reads-nothing / token-cannot-replay). |
| `migrations/0013_user_profile_soft_delete.sql` | A nullable `user_profile.deleted_at` (the soft-delete marker: null = active, set = closed). Backs account closure (`POST /api/v1/me/delete`) and the 90-day recovery window. | APPLIED TO PRODUCTION 2026-06-12 (owner-authorised, `DATABASE_URL`+asyncpg). Account deletion is a SOFT delete: it sets `deleted_at` and RETAINS the data for 90 days, during which the Coordinator REACTIVATES by signing back in (`POST /api/v1/me/reactivate` clears `deleted_at`); at 90 days the data is permanently deleted by the MANUAL operational purge below (no automated job). The hard-delete-due moment is COMPUTED in the api (`deleted_at + 90 days`, `GET /api/v1/me/account-status`), deliberately NOT a stored column. ADDITIVE + IDEMPOTENT; no RLS change needed (the `0001` `user_profile_select_own` / `user_profile_update_own` policies already scope the column to `auth.uid() = id`, so the owner reads, sets, and clears their OWN `deleted_at`). On closure the api also revokes the caller's active `card_record` rows (sets `revoked_at`), so a closed account stops exposing the recipient's data via a public share link. |
| `migrations/0018_subscription_entitlement.sql` | The subscription + entitlement foundation: `plan_tier` (the tiers + PRICES in pence + Stripe price ids + `active`), `feature_entitlement` (the per-tier value of each gated feature, PK `(feature_key, tier_key)`), `subscription` (one row per user: `tier_key`, `status`, `current_period_end`, the Stripe ids, `stripe_event_id UNIQUE`), and `billing_event` (the webhook idempotency ledger, `stripe_event_id` PK). Plus the SECURITY DEFINER RPC `public.apply_subscription_event(...)` (the webhook's only write path) and the board-split seed (recipients.max free=2/standard=3/premium=unlimited, card.pdf_export + themes paid). | APPLIED TO PRODUCTION 2026-06-12 (owner-authorised, `DATABASE_URL`+asyncpg) and SEEDED (`plan_tier` 3 / `feature_entitlement` 9). Backs the Subscription feature (`Docs/FeatureDecisions.md`, the Subscription DEFER entry; `HardRules/Api/Modules/Subscription.md`). **`plan_tier` + `feature_entitlement` are GLOBAL reference data** (read-for-authenticated, write-for-none, the 0002 posture), so prices AND the free/paid split live in DATA, owner-configurable with NO redeploy. **`subscription` is owner-SELECT-only with NO user write policy** (the self-grant fix at the DB layer, precondition 2): a user reads their own tier but can NEVER set it. **`billing_event` has RLS on and NO policy**, so a user touches nothing in the ledger. The webhook authenticates by STRIPE SIGNATURE (not a Supabase session, precondition 3) and writes ONLY through `apply_subscription_event` (revoked from `public`, NOT granted to anon/authenticated, reached only with the service-role key), which records the event id first and is IDEMPOTENT (a replayed Stripe event is a no-op, precondition 4). The live Stripe SDK calls are STUBBED in the api (`app/services/stripe_stub.py`, PENDING OWNER STRIPE KEYS). ADDITIVE + IDEMPOTENT (on-conflict-do-update seed); creates four tables + one function, alters no existing object. The real-Postgres RLS test (`tests/test_subscription_rls_real_postgres.py`, precondition 5) applies this migration verbatim to a DISPOSABLE Postgres and proves a user cannot update its own / read another's subscription while the RPC can write; it SKIPS unless `TIWANI_TEST_DATABASE_URL` names a throwaway DB (never production). |
...
3. **Verify** the tables, the RLS policies, and the `child_profile(user_id)`
   ...
   After applying `0015` + `0016` (the Shared-Child sharing substrate + feature
   layer), also re-run the sharing matrix proven by
   `tests/test_shared_child_rls.py` against the live DB: an active member reads the
   recipient's Continuity Card via `get_recipient_card_for_member` but reads NOTHING
   from `child_profile` / `lci_snapshot` / `alert_record` / `pulse_record` (the
   visibility CEILING); a viewer cannot mint / self-insert a membership / revoke; a
   revoked viewer reads nothing on the next request; a non-member reads nothing; a
   token cannot replay; a child share records a `share_consent` row; an adult share is
   blocked without a recorded adult consent. (That test bootstraps a throwaway
   Postgres itself when `RLS_TEST_DATABASE_URL` is set, so it is also the CI proof.)
